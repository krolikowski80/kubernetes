...
> Using namespaces to logically organize your Kubernetes resources is a best practice.
...
### Summary
> In this lab step, you learned that Kubernetes logs any output written by containers to standard output and standard error and stores those messages as logs. For legacy applications, you can use exec or cp to access log files when standard output and standard error aren't being used.
...
#### 5. In order to view the logs in S3, click the following button to access the lab’s cloud environment:


#### 6. Enter the following credentials created just for your lab session, and click Sign In:
...
> Warning: It takes a couple of minutes for the first logs to be sent to S3. If you don't see a logs folder, refresh the view after a minute.

 

####  8. Click one of the log files in the logs folder and then click Object actions > Open from the file details view:

> You may also need to allow the pop-up in your browser. For example in Chrome, click Pop-up blocked in the address bar followed by the blue URL beginning with https://s3....:
